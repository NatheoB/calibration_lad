...
```{r}
load("output/calib/out_20260113_dbhXbatot_phylogeny.Rdata")
```

```{r}
data_trace <- output_models_list$output %>% 
  purrr::map(~.x$output) %>% 
  setNames(output_models_list$output %>% purrr::map(~.x$id_model)) %>% 
  purrr::map(~.x %>% 
               purrr::map(~.x %>% 
                            getSample(coda = TRUE) %>%
  ...
  dplyr::select(id_model, chain, subchain, iteration,
                sigma_log, sigma_site_log, sigma_origin_log,
                intercept.angiosperm, intercept.gymnosperm,
                dbh.angiosperm, dbh.gymnosperm,
                batot.angiosperm, batot.gymnosperm,
                dbhXbatot.angiosperm, dbhXbatot.gymnosperm) %>% 
  tidyr::pivot_longer(!c(id_model, chain, subchain, iteration),
                      names_to = "parameter",
                      values_to = "estimate") %>% 
  
  dplyr::filter(id_model == 1)
```
...
data_trace %>% 
  ...
  labs(title = "Convergence of the sigma parameters",
       subtitle = "batot*dbh for angiosperms and gymnosperms") +
...
## Model for angiosperm

```{r}
data_trace %>% 
  
  dplyr::filter(grepl("angiosperm", parameter)) %>% 
  ...
  labs(title = "Convergence of the parameters for angiosperms") +
...
## Model for gymnosperm

```{r}
data_trace %>% 
  
  dplyr::filter(grepl("gymnosperm", parameter)) %>% 
  ...
  labs(title = "Convergence of the parameters for gymnosperms") +
...
tidyr::crossing(
  ...
  dplyr::mutate(
    lad_angiosperm = exp(intercept.angiosperm + 
                           dbh_std * dbh.angiosperm + 
                           batot_std * batot.angiosperm +
                           dbh_std * batot_std * dbhXbatot.angiosperm + 
                           0.5*variance_sum),
    
    lad_gymnosperm = exp(intercept.gymnosperm + 
                           dbh_std * dbh.gymnosperm + 
                           batot_std * batot.gymnosperm +
                           dbh_std * batot_std * dbhXbatot.gymnosperm + 
                           0.5*variance_sum)
  ) %>% 
  
  tidyr::pivot_longer(
    c(lad_gymnosperm, lad_angiosperm),
    values_to = "lad",
    names_to = "phylogeny",
    names_prefix = "lad_"
  ) %>% 
  ...
  dplyr::group_by(dbh_cm, compet_intensity, phylogeny) %>% 
  dplyr::summarise(
