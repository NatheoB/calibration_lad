...
# Preliminary analysis (filter the sensors)
...
# A simple Bayesian model (species and DBH effect)

```{r}
tar_load(models_setup)
tar_load(models_output)
tar_load(models_summary_pointwise)
tar_load(models_comparison)
tar_load(models_evaluation)
...
# )
# saveRDS(out_list, "output/calib/out_20251114_testRandom_5000it_1c.Rdata")
```
...
mod_id <- 2
out <- models_output[[mod_id]]
out$outputs <- list(out$outputs)

setup <- models_setup$setups[[mod_id]]
...
out$outputs %>%
  purrr::map(
    ~getSample(.x, 
               start = 0, end = NULL, thin = 1,
               # start = burning_period, end = NULL, thin = thinning,
  ...
  geom_line() +
  geom_vline(xintercept = burning_period, linetype = "dashed") +
  theme_bw()
```
...
## Species-specific intercept effect
...
out$outputs %>%
  purrr::map(
    ~getSample(.x, 
               start = 0, end = NULL, thin = 1,
               # start = burning_period, end = NULL, thin = thinning,
               whichParameters = grep(
                 "species",
                 colnames(out$outputs[[1]]$chain[[1]])),
               coda = TRUE) %>%
      purrr::map(~as.data.frame(.x) %>%
                   dplyr::mutate(iteration = row_number()) %>%
                   dplyr::relocate(iteration)) %>%
...
species_sample <- out$outputs %>%
  purrr::map(
    ~getSample(.x, start = burning_period, end = NULL, thin = thinning,
               whichParameters = grep(
                 "species",
                 colnames(out$outputs[[1]]$chain[[1]])),
               coda = TRUE) %>%
...
```{r}
species_sample_unlog <- out$outputs %>%
  purrr::map(
    ~getSample(.x, start = burning_period, end = NULL, thin = thinning,
               whichParameters = grep(
                 "species",
...
summaries_sp_unlog <- species_sample_unlog %>%
  dplyr::group_by(species) %>%
  dplyr::summarise(
    lower = quantile(estimate, probs = 0.025),
    median = quantile(estimate, probs = 0.5),
...
species_sample_unlog %>%
  ggplot(aes(x = estimate)) +
  geom_density() +
  facet_wrap(~species, scales = "free_y") +
  theme_bw() +
  geom_vline(data = summaries_sp_unlog, aes(xintercept = lower),
             linewidth = 1.05, color = "salmon", linetype = "dotted") +
  geom_vline(data = summaries_sp_unlog, aes(xintercept = median),
             linewidth = 1.05, color = "salmon") +
  geom_vline(data = summaries_sp_unlog, aes(xintercept = upper),
             linewidth = 1.05, color = "salmon", linetype = "dotted")
```
...
```{r}
models_evaluation$indicators %>% 
  dplyr::bind_rows(.id = "id_mod") %>% 
  dplyr::left_join(data_looic, by = "id_mod") %>% 
  dplyr::mutate(mod_label = paste0(id_mod, " | ", looic)) %>% 
  
  ggplot(aes(x = MAE, color = mod_label)) +
  geom_density() +
  theme_bw() +
  labs(color = "Model | LOOIC")
